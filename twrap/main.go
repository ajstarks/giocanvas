...
func work(title string, w, h int) {
	...
	for e := range win.Events() {
		if e, ok := e.(system.FrameEvent); ok {
			...
			var left float32 = 20
			canvas.Text(left, 90, subsize, "TextWrap(x, y, size, s, 60, red)", red)
			canvas.Text(left, 70, subsize, "TextWrap(x, y, size, s, 40, red)", green)
			canvas.Text(left, 40, subsize, "TextWrap(x, y, size, s, 20, red)", blue)
