...
func back(c *gc.Canvas, opts options, e []election, n *int, limit int) {
	*n--
	if *n < 0 {
		*n = limit - 1
	}
...
func forward(c *gc.Canvas, opts options, e []election, n *int, limit int) {
	*n++
	if *n > limit-1 {
		*n = 0
	}
...
func elect(title string, opts options, elections []election) {
	...
	for e := range win.Events() {
		...
		case system.FrameEvent:
			canvas := gc.NewCanvas(cw, ch, e)
			process(canvas, opts, elections[0])
